<template>
  <div style="max-width: 430px;
    ...
    <div id="course-container" :class="`${color} ${tocSmallWidth ? 'toc-small' : ''}`"
      :style="{ backgroundColor: colorConfigs[color]['background'] }" v-else>
      <div class="toc title" :style="{ backgroundColor: colorConfigs[color]['titleBackground'] }">
        <div class="input-span" contenteditable="plaintext-only" v-text="courseTitle"
          @blur="setValue('courseTitle', $event.target.innerHTML)"></div>
        <span class="left-bg"></span><span class="right-bg"></span> <span class="left-line"></span><span
          class="right-line"></span>
      </div>
      <template v-for="(item, index) in items" :key="index">
        <div class="toc item" v-if="!item.rootNode">
          <span class="index" :style="{ color: colorConfigs[color]['tocIndexColor'] }"
            @dblclick="() => { item.rootNode = !item.rootNode }">{{ startNumber(index) }}</span>
          <span :style="{ color: colorConfigs[color]['tocTitleColor'] }" class="input-span"
            contenteditable="plaintext-only" v-text="item.text"
            @blur="setValue2(item, 'text', $event.target.innerHTML)"></span>
        </div>
        <template v-else>
          <div class="toc root-title" :style="{ backgroundColor: colorConfigs[color]['moreBackground'] }">
            <span class="input-span" contenteditable="plaintext-only" v-text="item.text"
              @dblclick="() => { item.rootNode = !item.rootNode }"
              @blur="setValue2(item, 'text', $event.target.innerHTML)"></span>
          </div>
        </template>
      </template>
      <div class="toc more" :style="{ backgroundColor: colorConfigs[color]['moreBackground'] }">
        ...
        <div class="input-span" contenteditable="plaintext-only" v-text="courseMore"
          @blur="setValue('courseMore', $event.target.innerHTML)"></div>
        <span class="left-bg"></span><span class="right-bg"></span><span class="left-line"></span><span
          class="right-line"></span>
      </div>
      <div style="margin-bottom: 10px;" v-if="showQRCode">
        ...
        <div class="footer-center">
          <span contenteditable="plaintext-only" class="input-span" v-text="QRcodeSelect.message"
            @blur="setValue2(QRcodeSelect, 'message', $event.target.innerHTML)" style="font-size: 12px;"
            :style="{ color: colorConfigs[color]['moreBackground'] }"></span>
        </div>
        <!-- 居中显示二维码 -->
        <div class="qrcode">
          <div v-if="QRcodeSelect.isFlie && QRcodeSelect.file">
            <img v-for="(f, index) in QRcodeSelect.file" :key="index"
              style="width: 100px;height: 100px;border-radius: 4px;object-fit: cover;" :src="f"
              @click="showQRcodeChange" />
          </div>
          <a-qrcode v-else :size="100" errorLevel="H" :value="QRcodeSelect.data"
            :color="colorConfigs[color]['moreBackground']" @click="showQRcodeChange" />
        </div>
      </div>
    </div>
    <div id="options">
      <div v-if="!image">
        ...
        <a-row>
          <a-col :span="6">
            <a-popconfirm title="确认清空填写的内容吗?" ok-text="确认" cancel-text="取消" @confirm="clearItem()">
        ...
        <a-row v-if="batchInput.show">
          <a-col :span="24">
            ...
            <a-textarea v-model:value="batchInput.data" @blur="batchInputOk" @change="batchInputItem"
              placeholder="一行作为一个标题，可粘贴整理好的换行内容，有序号会自动去除" :auto-size="{ minRows: 5, maxRows: 20 }"></a-textarea>
          </a-col>
        </a-row>
        <p v-if="!batchInput.show" style="text-align: center;">点击内容编辑，双击序号切换为章节，选择主题，生成图片</p>
        <a-row>
          <a-col :span="24" style="text-align: center;">
        ...
        <div class="footer-center">
          <a-radio-group style="width: 100%;" v-model:value="color" @change="saveData">
            <a-radio-button v-for="(value, key) in colorConfigs" :key="key"
              :style="{ backgroundColor: value.background, color: key === 'morebule' ? '#fff' : '' }" :value="key">{{
      value.name
    }}</a-radio-button>
          </a-radio-group>
        </div>
        <a-row v-if="showQRCode">
          ...
          <a-col :span="24">
            <a-input class="footer-center" placeholder="输入链接更新二维码" v-model:value="QRcodeSelect.data"
              @change="() => { QRcodeSelect.isFlie = false; saveData() }"></a-input>
            <a-upload :show-upload-list="false" class="footer-center" style="display: block;" :maxCount="1"
              accept="image/*" @change="QRcodeFileChange" :before-upload="beforeQRcodeUpload">
          ...
          <a-col :span="24">
            ...
            <template v-for="(item, index) in savedTocs.data" :key="index">
              <div style="width: 90%;margin: 5px 5%;">
                <a-popconfirm title="确认替换当前内容和配置吗?" ok-text="确认" cancel-text="取消" @confirm="useThisConfig(item)">
                  <a-button style="width: 85%;" type="dashed">{{ item.configName }}</a-button>
                </a-popconfirm>
                <a-popconfirm title="删除此配置吗?" ok-text="确认" cancel-text="取消" @confirm="deleteThisConfig(index)">
